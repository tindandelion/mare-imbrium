...
Visualization of lunar surface
...
- **Modules:** Prefer the post-2018 layout for directory modules: **`parent.rs` + `parent/child.rs`** (e.g. `geometry.rs` + `geometry/model_mesh.rs`, `meshes.rs` + `meshes/cube.rs`). Avoid adding new **`mod.rs`** files unless there is a compelling reason. **Private submodules, flat public surface:** implementation files stay **`mod`**-private; re-export types and functions at the parent boundary (**`geometry::ModelMesh`**, **`meshes::cube()`**, …).
